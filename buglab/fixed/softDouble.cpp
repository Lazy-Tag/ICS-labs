...
uint64_t add(uint64_t lhs, uint64_t rhs){
    ...
    if(ansexp == 0 && ansf >= (1ll << 52))
        ++ansexp;

    assert((ansexp != 0 && ansf < (1ll << 53)) || (ansexp == 0 && ansf < (1ll << 52)));

    if(ansexp >= ((1 << 11) - 1)) // overflow
        ans = INF;
    else
        ans = ansexp << 52 | (ansf & ((1ll << 52) - 1));

    ans |= (1ll << 63) & lhs; // Add sign
    return ans;
}
...
uint64_t subtract(uint64_t lhs, uint64_t rhs){
    ...
    int ediff = std::min(Exp(lhs) - Exp(rhs), 63ll);
    assert(ediff >= 0);
...
uint64_t multiply(uint64_t lhs, uint64_t rhs){
    ...
    // Adjusting exp
    while(ansexp < 0 || ansf >= (1ll << 54)){
        ++ansexp;
        roundup |= ansf & 1;
        ansf >>= 1;
    }
    while(ansexp > 0 && (ansf & (1ll << 53)) == 0){
        --ansexp;
        ansf <<= 1;
    }
    while(ansexp < 0){
    ...
    ans |= ((1ll << 63) & lhs) ^ ((1ll << 63) & rhs); // Add sign
    return ans;
}
...
uint64_t divide(uint64_t lhs, uint64_t rhs){
    ...
    if(isZero(rhs)){ // divided by zero
        if(isZero(lhs))
            ...
            return INF | ((lhs ^ rhs) & (1ll << 63));
    }
    if(isINF(rhs)){  // divided by INF
        if(isINF(lhs))
            return NaN;
        else
            return ((1ll << 63) & (lhs ^ rhs)); // signed zero
    }
    if(isINF(lhs))
        return INF | ((lhs ^ rhs) & (1ll << 63)); // INF/INF handled, other return INF
...
char* write_to_string(uint64_t x){
    ...
    if(isNaN(x))
        ...
        sprintf(ans, "%.1200lf",_mm_set_pi64x(x));
    return ans;
}
...
inline uint64_t Negative(uint64_t x){
    return isNaN(x)? x : (x ^ (1ll << 63));
}

inline int64_t Exp(uint64_t x){
    return (x >> 52) & ((1ll << 11) - 1);
}
...
inline uint64_t Fraction(uint64_t x){
    if(Exp(x) != 0)
        return 1ll << 52 | (x & ((1ll << 52) - 1));
    else
        return (x & ((1ll << 52) - 1)) << 1; // normalize subnormal
}
